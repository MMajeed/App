...
void CDrawableTex2D_DX11::init(ID3D11Device* device, ID3D11DeviceContext* contextDevice, UINT width, UINT height, bool hasColorMap, DXGI_FORMAT colorFormat)
{
	...
	md3dDevice = device;
	md3dDeviceContext = contextDevice;
...
void CDrawableTex2D_DX11::buildDepthMap()
{
	...
	//hr(
	HRESULT hrOK = md3dDevice->CreateTexture2D(&texDesc, 0, &depthMap);
   ...
   // HR(
	hrOK = md3dDevice->CreateDepthStencilView(depthMap,&dsvDesc, &mDepthMapDSV);
   ...
   // HR(
	hrOK = md3dDevice->CreateShaderResourceView(depthMap, &srvDesc, &mDepthMapSRV);
...
void CDrawableTex2D_DX11::end()
{
    ...
    if( mColorMapSRV )
	{
		md3dDeviceContext->GenerateMips(mColorMapSRV);
	}
